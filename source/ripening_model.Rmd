...
pheno_data <- pheno_data %>% select(apple_id, brix_17_harv, brix_17_stor, firmness_avg_17_harv, firmness_avg_17_stor, date_jul_17_harv)

nac_pheno_data <- left_join(nac_data, pheno_data, by = "apple_id")

median_summary <- nac_pheno_data %>%
  group_by(nac_alleles) %>%
  summarise(firm_harv = median(firmness_avg_17_harv, na.rm = T), firm_stor = median(firmness_avg_17_stor, na.rm = T), brix_harv = median(brix_17_harv, na.rm = T), brix_stor = median(brix_17_stor, na.rm = T), harv_date = median(date_jul_17_harv, na.rm = T))
