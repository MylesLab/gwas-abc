...
pheno_cor_pc1 <- select(pcs_pheno,PC1, release_year:bin_world) %>% mutate_all(as.numeric)
...
write.csv(pc1_vs_phenos,file = "../outputs/pc1_vs_phenos.csv", quote = F, row.names = F)

#Correlate PC2 with phenotypes

pheno_cor_pc2 <- select(pcs_pheno,PC2, release_year:bin_world) %>% mutate_all(as.numeric)
...
phenolics <- ggplot(abc_pcs, aes(x=date_jul_17_harv, y=PC2))+
  geom_point(aes(colour=date_jul_17_harv),size=1, alpha=0.8)+
  theme_bw()+
  coord_fixed()+
  scale_color_viridis(option = "magma", direction = -1)+
