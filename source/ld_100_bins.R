...
#try bins based on physical distances instead of number of SNPs.
ld_table = read.table("abc_combined_maf001_sort_vineland_imputed_pheno_hetero90_maf001_noctgs_ld.ld")
ld_table <- as.data.frame(ld_table)
View(c(ld_table[,"SNP_A"],ld_table[,"SNP_B"]))
snp_count = length(unique(c(as.character(ld_table[, "SNP_A"]), as.character(ld_table[, "SNP_B"]))))


chr_a <- ld_table[, "SNP_A"]
chr_b <- ld_table[, "SNP_B"]
...
samp_bins = seq(0, 1000000, by = 100)
median_r2 = mean_r2 = c()
...
pdf("abc_ld_decay_100bins.pdf", width = 5, height =5)
ld_100 <- ggplot(ld_stats, aes(x=V3, y= mean_r2))+ 
  ...
  theme_classic()
ggsave("abc_ld_decay_100bins.pdf", plot = ld_100)
