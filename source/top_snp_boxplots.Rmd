...
date_nac <- ggplot(date, aes(x=factor(nac, levels = c("2", "1", "0")), y=date_jul_17_harv)) + 
  ...
  stat_summary(fun=median, geom = "crossbar",
               ...
               size=0.2,
               show.legend=FALSE)+
  scale_colour_manual(values=c("#fc8961","#d8456c","#932b80"))+
  theme_bw()+
  stat_n_text() + 
...
top_snp_genos <- read_csv("~/Documents/myles_lab/abc-gwas/outputs/top_snp_genos.csv")
#keep SNPs for phenolics
firm_stor_snp <- top_snp_genos %>% select(apple_id, firm_stor_top_chr16, firm_stor_top_chr16_alleles)

#load phenotype data
geno_pheno_meta_data <- read_csv("../outputs/geno_pheno_meta_data.csv")
firm_stor_pheno <- geno_pheno_meta_data %>% select(apple_id, firmness_avg_17_stor)

#join pheno data and snp data
firm_stor_snp <- left_join(firm_stor_snp, firm_stor_pheno, by = "apple_id")

#remove rows without phenotype data
firm_stor_snp <- firm_stor_snp %>% filter(!is.na(firmness_avg_17_stor))

#16_9235229_tassel
firm_stor <- ggplot(firm_stor_snp, aes(x=factor(firm_stor_top_chr16), y=firmness_avg_17_stor)) + 
  geom_jitter(aes(colour=as.factor(firm_stor_top_chr16)), position = position_jitterdodge(jitter.width = 1, dodge.width =0.8),stroke=0, alpha = 0.7, size=2, show.legend=FALSE) +
  stat_summary(fun=median, geom = "crossbar",
               position = position_dodge(width=0.8),
               width=0.5,
               size=0.2,
               show.legend=FALSE)+
  scale_colour_manual(values=c("#932b80","#d8456c","#fc8961"))+
  theme_bw()+
  stat_n_text() + 
  scale_x_discrete(labels = c('AA','AC','CC'))+
  scale_y_continuous(limits = c(0,15))+
  labs(x="Chr16:9235229", y="Firmness after storage (kg/cm2)")+
  theme(panel.border = element_blank(), axis.text=element_text(colour = "black", size=10), axis.title=element_text(size = 10, face = "bold"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), plot.title = element_text(size = 9))
ggsave(firm_stor, file = "../figures/firm_stor.pdf", width = 7, height = 10, units = "cm")
